        ...
        <ScrollView style={{ alignSelf: 'stretch' }}>
          {
            this.state.contents
              ? this.state.contents.map((param, i) => {
                return (
                  <DropDownItem style={{flexDirection:'row'}}
                    key={i}
                    ...
                    header={
                      <View style={styles.header}>
                          <Text style={{fontSize: 16,color: '#fff',}}>{param.title}</Text>
                      </View>
                    }>
                    <Text style={[styles.txt,{fontSize: 20, backgroundColor:'#00BFFF'},]}>{param.body}</Text>
                  </DropDownItem>
                );
              })
              : null
